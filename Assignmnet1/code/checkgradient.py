...
def form_parameters(w,b,layer_dims):
    parameters = {}
    L = len(layer_dims)
    lower = 0
    for l in range(1, L):
        upper = layer_dims[l-1] + lower
        w_list = read_parameters_from_file(w, lower, upper)
        parameters['W' + str(l)] = w_list
        lower = upper
        b_list = read_parameters_from_file(b,l-1,l)
...
def checkgradient_network_1():
    ...
    w = "../data/question_2_2c/w-100-40-4.csv"
    b = "../data/question_2_2c/b-100-40-4.csv"
    parameters = form_parameters(w,b,layers_dims)
    grads = compute_gradients(X, Y, parameters)
    dw = "../Question_2/dw-100-40-4.csv"
    db = "../Question_2/db-100-40-4.csv"
    name = [dw, db]
...
def checkgradient_network_2():
    ...
    X, Y = load_data_from_txt()
    w = "../data/question_2_2c/w-28-6-4.csv"
    b = "../data/question_2_2c/b-28-6-4.csv"
    parameters = form_parameters(w, b, layers_dims)
    grads = compute_gradients(X, Y, parameters)
    dw = "../Question_2/dw-28-6-4.csv"
    db = "../Question_2/db-28-6-4.csv"
    name = [dw, db]
    write_grads_to_csv(name,layers_dims, grads)
...
def checkgradient_network_3():
    ...
    X, Y = load_data_from_txt()
    w = "../data/question_2_2c/w-14-28-4.csv"
    b = "../data/question_2_2c/b-14-28-4.csv"
    parameters = form_parameters(w, b, layers_dims)
    grads = compute_gradients(X, Y, parameters)
    dw = "../Question_2/dw-14-28-4.csv"
    db = "../Question_2/db-14-28-4.csv"
    name = [dw, db]
    write_grads_to_csv(name,layers_dims,grads)
